...
class WC_BOGOF_Choose_Gift {
	...
	public static function init_hooks() {
		...
		self::$cart_hash = isset( $_REQUEST['wc_bogo_refer'] ) ? wc_clean( $_REQUEST['wc_bogo_refer'] ) : false;
	...
	private static function is_page( $a, $b = false ) {
		$b = ! empty( $b ) ? $b : wc_clean( $_SERVER['REQUEST_URI'] );
	...
	private static function is_ajax() {
		$is_ajax = is_ajax() || ( isset( $_SERVER['HTTP_X_REQUESTED_WITH'] ) && 'xmlhttprequest' === strtolower( $_SERVER['HTTP_X_REQUESTED_WITH'] ) );
		if ( $is_ajax ) {
	...
	/**
	 * Filter the variatons for the choose your gift action.
	 *
