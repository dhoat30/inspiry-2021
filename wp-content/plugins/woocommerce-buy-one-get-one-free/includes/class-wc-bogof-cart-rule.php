...
class WC_BOGOF_Cart_Rule {
	...
	protected function add_free_product_to_cart( $qty ) {
		...
		if ( $product_id ) {
			$cart_item_key = WC()->cart->add_to_cart( $product_id, $qty, 0, array(), WC_BOGOF_Cart::set_cart_item_free( array(), $this->get_id() ) );
	 ...
	 * @param bool  $raw Get raw value? Do not remove discounts qty if raw s true.
	 * @return int
	 */
